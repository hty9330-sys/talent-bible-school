...
async function completeBibleLesson(studentId) { if (!isStaff()) return setMessage("선생님 권한이 필요합니다."); const student = getStudent(studentId); if (!student) return setMessage("아이를 먼저 등록하세요."); const { error } = await state.client.rpc("complete_bible_lesson", { p_student_id: student.id, p_lesson_title: bibleLesson.title, p_verse_ref: bibleLesson.verseRef, p_completed_items: bibleLesson.words.length + bibleLesson.lines.length, p_talents_awarded: 2 }); if (error) return setMessage(error.message); await loadRemoteData(); state.selectedStudentId = student.id; state.message = `${student.name}의 성경 학습을 완료하고 2 달란트를 기록했습니다.`; state.view = "student"; render(); }
