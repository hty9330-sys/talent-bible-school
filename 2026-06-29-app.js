...
function bibleView() { const student = getStudent(state.selectedStudentId); const lesson = currentBibleLesson(); return `<section class="stack bible-view"><div class="section-heading"><h2>성경 영어 학습</h2><span>${escapeHtml(lesson.verseRef)}</span></div><div class="form-panel bible-lesson-panel">${student ? studentSelect(student.id) : `<p class="empty">아이를 먼저 등록하거나 연결하세요.</p>`}<p class="eyebrow">${escapeHtml(lesson.title)}</p><h2>오늘의 말씀과 단어</h2><p class="empty">관리자가 입력한 이번 주 말씀에서 어려운 단어를 자동으로 뽑습니다.${isStaff() ? " 완료하면 달란트가 기록됩니다." : ""}</p><div class="word-grid">${lesson.words.map((item) => `<div class="word-card"><div><strong>${escapeHtml(item.word)}</strong><small>${escapeHtml(item.sound)}</small></div><p>${escapeHtml(item.meaning)}</p><button class="audio-button" type="button" data-speak="${escapeHtml(item.word + ". " + item.example)}">듣기</button></div>`).join("")}</div></div><div class="feed verse-panel"><h2>말씀 한 줄씩 듣기</h2>${lesson.lines.map((line, index) => `<div class="verse-line"><span>${index + 1}. ${escapeHtml(line)}</span><button class="audio-button" type="button" data-speak="${escapeHtml(line)}">듣기</button></div>`).join("")}<div class="action-row"><button class="secondary-button" type="button" data-speak="${escapeHtml(lesson.lines.join(" "))}">전체 말씀 듣기</button>${isStaff() ? `<button class="primary-button" type="button" data-action="complete-bible">학습 완료 +2</button>` : ""}</div></div>${bibleFeed("최근 성경 학습", state.bibleRecords.slice(0, 6))}</section>`; }
function adminView() {
...
async function updateWeeklyBibleLesson(payload) { if (!isAdmin()) return setMessage("관리자만 성경 구절을 업데이트할 수 있습니다."); const { error: updateError } = await state.client.from("weekly_bible_lessons").update({ is_active: false }).eq("is_active", true); if (updateError) return setMessage(updateError.message); const { error } = await state.client.from("weekly_bible_lessons").insert({ title: payload.title, verse_ref: payload.verseRef, verse_text: payload.verseText, created_by: state.session.user.id, is_active: true }); if (error) return setMessage(error.message); await loadRemoteData(); state.message = "이번 주 성경 구절을 저장했습니다."; state.view = "bible"; render(); }
...
function bindEvents(root) {
  ...
  root.querySelector("#bible-lesson-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); await updateWeeklyBibleLesson({ title: String(form.get("title") || "").trim(), verseRef: String(form.get("verseRef") || "").trim(), verseText: String(form.get("verseText") || "").trim() }); });
