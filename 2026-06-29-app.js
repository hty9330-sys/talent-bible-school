...
function awardView() {
  ...
  const allowedAmounts = isAdmin() ? [1, 2, 3, 4, 5] : [1, 2];
...
async function awardTalent(payload) { const student = getStudent(payload.studentId); if (!student) { setMessage("아이를 먼저 등록하세요."); return false; } if (!isStaff()) { setMessage("선생님 권한이 필요합니다."); return false; } const amount = Number(payload.amount); const allowedAmounts = isAdmin() ? [1, 2, 3, 4, 5] : [1, 2]; if (!allowedAmounts.includes(amount)) { setMessage(`지급 가능한 달란트는 ${allowedAmounts.join(", ")} 뿐입니다.`); return false; } const { error } = await state.client.from("talent_transactions").insert({ student_id: student.id, teacher_id: state.session.user.id, amount, reason: payload.reason, memo: payload.memo || null }); if (error) { setMessage(error.message); return false; } await loadRemoteData(); state.selectedStudentId = student.id; state.message = `${student.name}에게 ${amount} 달란트를 지급했습니다.`; return true; }
