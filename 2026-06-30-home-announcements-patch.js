(() => {
  ...
  layout = function patchedChurchLayout(content) {
    ...
    return `<div class="app-shell"><header class="topbar"><div><p class="church-title">들사람교회</p><p class="eyebrow">달란트 성경학교</p><h1>아이들의 말씀과 성장을 함께 기록해요</h1></div><div class="topbar-actions">${state.session ? `<button class="ghost-button" type="button" data-action="logout">로그아웃</button>` : `<button class="ghost-button" type="button" data-view="login">로그인</button>`}<button class="ghost-button" type="button" data-view="settings">설정</button></div></header><div class="notice">${modeText}${state.loading ? " · 불러오는 중" : ""}</div>${state.message ? `<div class="toast">${escapeHtml(state.message)}</div>` : ""}<main>${content}</main><nav class="bottom-nav" aria-label="주요 메뉴">${navButton("home", "홈")}${navButton("students", "아이")}${isStaff() ? navButton("award", "지급") : ""}${navButton("bible", "성경")}${isAdmin() ? navButton("admin", "관리") : ""}</nav></div>`;
  };
