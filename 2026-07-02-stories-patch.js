(() => {
  ...
  function storiesAnnouncementPanel() {
    ...
    return `<section class="feed announcement-feed"><h2>교회소식</h2>${activeItems.length === 0 ? `<p class="empty">등록된 교회소식이 없습니다.</p>` : ""}${activeItems.map((item) => `<div class="feed-item announcement-item"><span><b>${escapeHtml(item.title)}</b></span><small>${formatDate(item.created_at)}</small><p style="white-space:pre-wrap;">${escapeHtml(item.body)}</p></div>`).join("")}</section>`;
  }
