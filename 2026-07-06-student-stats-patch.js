...
<h3>최근 활동</h3>
${activity.length === 0 ? `<p class="empty">선택한 기간에 활동이 없습니다.</p>` : activity.map((a) => `<div class="feed-item"><small>${formatDate(a.at)}</small><p>${escapeHtml(a.text)}</p></div>`).join("")}
</div>`;
  }
  ...
  studentView = function patchedStudentView() {
    ...
    if (state.studentTab === "notes" && staff) {
      body = noteFeed("선생님 메모 기록", state.notes.filter((item) => item.student_id === student.id));
    } else if (state.studentTab === "stats" && staff) {
      body = studentStatsHtml(student);
    } else {
      body = transactionFeed("전체 달란트 지급 기록", state.transactions.filter((item) => item.student_id === student.id));
    }
